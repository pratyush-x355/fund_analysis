...
def demo_hmm_detector_monthly(monthly_data, price_column='Adj Close', date_column='Date'):
    ...
    # Show sample results
    print(f"\nSample results (first 10 months):")
    display_cols = [date_column, price_column, 'returns', 'hidden_state']
    available_cols = [col for col in display_cols if col in result_data.columns]
    print(result_data[available_cols].head(10))
    
    # Get basic regime statistics
    try:
        ...
        for regime in stats['regime_details']:
            print(f"Regime {regime['regime_id']}: "
                  ...
                  f"Avg Return: {regime['mean_return']:.4f}, "
                  f"Volatility: {regime['variance']**0.5:.4f}")
    except Exception as e:
        print(f"Note: Detailed statistics not available - {str(e)}")
    ...
    print(f"\nReturning DataFrame with regime assignments for each month")
    
    return final_result

def demo_hmm_detector_monthly_2(monthly_data, price_column='Adj Close', date_column='Date'):
    """
    Simplified HMM regime detector for monthly data with fixed 4 regimes.
